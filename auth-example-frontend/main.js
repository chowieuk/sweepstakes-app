// temporarily taken from github.com/go-pkgz/auth/_example
// used for debugging / dev purposes. Thanks guys!
...
function loginAnonymously(username) {
  return fetch(
    `/auth/anonymous/login?id=sweepstakes&user=${encodeURIComponent(username)}`
  );
}
...
function sendEmailAuthData(username, email) {
  return req(
    `/auth/email/login?id=sweepstakes&user=${encodeURIComponent(
      username
...
function getMongoLoginForm(onSubmit) {
  const form = document.createElement("form");

  const usernameInput = document.createElement("input");
  usernameInput.type = "text";
  usernameInput.placeholder = "Username";
  usernameInput.className = "mongo-form__input";

  const passwordInput = document.createElement("input");
  passwordInput.type = "password";
  passwordInput.placeholder = "Password";
  passwordInput.className = "mongo-form__input";

  const submit = document.createElement("input");
  submit.type = "submit";
  submit.value = "Log in";
  submit.className = "mongo-form__submit";
  ...
  // // onValueChange(usernameInput.value);

  // usernameInput.addEventListener("username", e => {
  //   onValueChange(e.target.value);
  // });
  ...
  return getProviders().then(providers =>
    providers.map(prov => {
      ...
      if (prov === "anonymous") {
        ...
        mongoForm.style.display = "none";
        mongoForm.className = "mongo-form login__mongo-form";

        a.appendChild(mongoForm);
      } else if (prov === "email") {
        a = document.createElement("span");
        a.dataset.provider = prov;
        a.className = "login__prov";
